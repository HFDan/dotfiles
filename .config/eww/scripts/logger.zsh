...
function create_cache() {
    ...
    if [[ $summary != "Volume:" ]]; then
        ...
        then
            if [[ ${#DUNST_BODY} -ge 40 ]]
            then
                fixed_body=$(echo $DUNST_BODY | cut -c 1-40)
                body=$(python -c "print('${fixed_body}' + ' ...')")
            else
                body=$DUNST_BODY
            fi
...
function make_literal() {
    ...
    [[ "$caches" == "" ]] \
    && print '(box :class "notifications-empty-box" :height 400 :orientation "vertical" :space-evenly "false" (label :vexpand true :valign "center" :wrap true :class "notifications-empty-label" :text "ノティフィケションはありません :("))' \
    || print "(scroll :height 400 :vscroll true (box :orientation 'vertical' :class 'notification-scroll-box' :spacing 20 :space-evenly 'false' $caches))"
}
